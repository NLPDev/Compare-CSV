...
top_10_concat_features, all_f_imp_concat = get_feature_imp(MLPClassifier(hidden_layer_sizes=(500,300,100,)), X_train, y_train, X_test, y_test)
...
mlp = MLPClassifier(hidden_layer_sizes=(500,300,100,))
mlp.fit(X_train_pos, y_train)
pred = mlp.predict(X_test_pos)
pred_proba = mlp.predict_proba(X_test_pos)


def mlp_model(X_train, y_train, X_test, y_test, results):
    mlp = MLPClassifier(hidden_layer_sizes=(500,300,100,))
    mlp.fit(X_train, y_train)
    Y_pred = mlp.predict(X_test)
    results['MLP'] = {}
    results['MLP']['Accuracy'] = accuracy_score(y_test, Y_pred)
    results['MLP']['cm'] = confusion_matrix(y_test, Y_pred)
    results['MLP']['f1_macro'] = f1_score(y_test, Y_pred, average='macro')
    results['MLP']['f1_class'] = f1_score(y_test, Y_pred, average=None)
    results['MLP']['pred_prob'] = mlp.predict_proba(X_test)
...
results = dict()
mlp_model(X_train_pos, y_train, X_test_pos, y_test, results)
...
draw_learning_curve(MLPClassifier(hidden_layer_sizes=(500,300,100,)), fm_bd_model.drop(['B_F1_Bool_Result', 'Event_Date'], axis=1), fm_bd_model['B_F1_Bool_Result'], 'MLP')
