...
top_10_concat_features, all_f_imp_concat = get_feature_imp(SVC(probability=True), X_train, y_train, X_test, y_test)
...
svc.fit(X_train_pos, y_train)
pred = svc.predict(X_test_pos)
pred_proba = svc.predict_proba(X_test_pos)


def svc_model(X_train, y_train, X_test, y_test, results):
    svc = SVC(probability=True)
    svc.fit(X_train, y_train)
    Y_pred = svc.predict(X_test)
    results['SVC'] = {}
    results['SVC']['Accuracy'] = accuracy_score(y_test, Y_pred)
    results['SVC']['cm'] = confusion_matrix(y_test, Y_pred)
    results['SVC']['f1_macro'] = f1_score(y_test, Y_pred, average='macro')
    results['SVC']['f1_class'] = f1_score(y_test, Y_pred, average=None)
    results['SVC']['pred_prob'] = svc.predict_proba(X_test)
...
results = dict()
svc_model(X_train_pos, y_train, X_test_pos, y_test, results)
